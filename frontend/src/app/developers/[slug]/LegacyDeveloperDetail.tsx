// Transaction-derived developer detail (brand-heuristic text slugs). Preserved
// for links from HomeFeaturedDevelopers which still key on the brand slug.
import Link from 'next/link';
import { Building2, Award, ArrowUpRight } from 'lucide-react';
import { Container } from '@/components/Container';
import { Breadcrumbs } from '@/components/nav/Breadcrumbs';
import { formatNumber } from '@/lib/format';
import type { DeveloperDetail, DeveloperProjectRow } from '@/lib/types';

export function LegacyDeveloperDetail({ detail }: { detail: DeveloperDetail }) {
      ...
      <div className="border-b border-border">
        <Container>
          <div className="pt-4 pb-3">
            <Breadcrumbs items={[{ label: 'Developers', href: '/developers' }, { label: name }]} />
            <div className="mt-2 flex items-end justify-between gap-3 flex-wrap">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs text-fg-muted">
                  Active since {summary.earliest_year ?? '—'} · {summary.areas_served} Dubai areas
                </p>
              </div>
              <div
              ...
              >
                ...
                <div className="text-[10px] uppercase tracking-wider mt-0.5">{summary.track_record_label}</div>
              </div>
            </div>
          </div>
        </Container>
          ...
          {summary.top_areas.length > 0 && (
            <section className="surface-card p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {summary.top_areas.map((a) => <span key={a} className="pill">{a}</span>)}
              </div>
            </section>
          )}

          {active.length > 0 && (
            <ProjectsList title={`Active projects · under construction (${active.length})`} rows={active} accent />
          )}
          {completed.length > 0 && <ProjectsList title={`Completed projects (${completed.length})`} rows={completed} />}
          {comingSoon.length > 0 && <ProjectsList title={`Coming Soon (${comingSoon.length})`} rows={comingSoon} />}
...
function ProjectsList({ title, rows, accent }: { title: string; rows: DeveloperProjectRow[]; accent?: boolean }) {
  return (
          ...
          <tbody>
            {rows.map((p) => (
              <tr key={p.project_slug}>
                ...
                <td className={`num ${accent ? 'text-accent' : ''}`}>{formatNumber(p.total_units)}</td>
                ...
                <td className="text-right">
                  {p.is_offplan ? (
                    <Link href={`/offplan/${p.project_slug}`} className="inline-flex items-center gap-1 text-[11px] text-accent hover:underline">
