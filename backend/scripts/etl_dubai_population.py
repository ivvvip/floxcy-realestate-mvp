...
def write_to_db(rows: list[dict]) -> dict:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # ---- Match step: count canonical-area hits ----
            cur.execute(
                """
                SELECT COUNT(DISTINCT p.community_code) AS matched
            ...
            stats["canonical_matches"] = matched
            stats["canonical_match_rate"] = (
                round(matched / inserted * 100, 1) if inserted else 0
            )
